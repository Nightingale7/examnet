...
from pd.algorithm_parameters import *
...
def get_sut(mode=None):
    sys.path.append(os.path.split(os.path.dirname(__file__))[-1])
    from pendulum_sut import Pendulum
    from pendulum_benchmark import pendulum_model

    sut_parameters = {
        "inputs": ["i0", "i1"],
        "outputs": ["PROB_GT_OVER_ETA", "PROB_ERROR_X_OVER_ALPHA"],
        "input_range": [[0, 1], [0, 1]],
    ...
    return Pendulum(sut_parameters)

def build_specification(selected_specification):
    if selected_specification == "PD":
        sim_end_time = 15
        epsilon = 0.1
        gamma = 0.9

        # TODO: Write this properly.
        specification = "not( (always[0,{}] (PROB_GT_OVER_ETA <= {})) and (eventually[0,{}] PROB_ERROR_X_OVER_ALPHA >= {}) )".format(sim_end_time, epsilon, sim_end_time, gamma)
