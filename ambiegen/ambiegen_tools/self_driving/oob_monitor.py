...
class OutOfBoundsMonitor:
    ...
    def oob_distances(self, wrt="right"):
        ...
        car_point = Point(self.vehicle_state_reader.get_state().pos)
        # New point. This is the center of the car. We do not use this as it is skewed to the left from the middle of
        # the right lane.
        #car_polygon = self._get_car_bbox_polygon()
        #car_point = car_polygon.centroid

        distance_new = self.road_polygon.right_polyline.distance(car_point)
        foo = Point(self.vehicle_state_reader.get_state().pos)
        distance_old = self.road_polygon.right_polyline.distance(foo)
